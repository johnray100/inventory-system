...
import {
    ...
    ListItemText, Typography, Divider, Tooltip, Avatar, Collapse,
} from "@mui/material";
            ...
            <Box key={item.label}>
                <Tooltip title={!open ? item.label : ""} placement="right">
                    <ListItem disablePadding sx={{ mb: 0.5 }}>
                        <ListItemButton
                            onClick={() => hasChildren ? handleToggleMenu(item.label) : navigate(item.path)}
                            sx={{
                                borderRadius: 2,
                                px: isChild ? 2.5 : 1.5,
                                py: isChild ? 0.8 : 1,
                                ...
                                bgcolor: (isActive && !hasChildren) || (isActive && hasChildren && open) ? SIDEBAR_STYLES.activeBg : "transparent",
                                color: (isActive && !hasChildren) || (isActive && hasChildren && open) || isChildActive ? SIDEBAR_STYLES.activeText : SIDEBAR_STYLES.text,
                                transition: "all 0.2s",
                                "&:hover": {
                                    bgcolor: (isActive && !hasChildren) || (isActive && hasChildren && open) ? SIDEBAR_STYLES.activeBg : SIDEBAR_STYLES.hoverBg,
                                    color: "white",
                                },
                                ...(((isActive && !hasChildren) || (isActive && hasChildren && open)) && {
                        ...
                        >
                            ...
                            >
                                {isChild ? <FiberManualRecordIcon /> : item.icon}
                            </ListItemIcon>
                            {open && (
                                <ListItemText
                                    primary={item.label}
                                    primaryTypographyProps={{ 
                                        ...
                                        fontWeight: (isActive && !hasChildren) || isChildActive ? 600 : 400 
                                    }}
                                />
                            )}
                            {open && hasChildren && (
                                isMenuOpen ? <ExpandLess sx={{ fontSize: 18 }} /> : <ExpandMore sx={{ fontSize: 18 }} />
                            )}
                        </ListItemButton>
                    </ListItem>
                </Tooltip>
            ...
            >
                ...
                {open && (
                    <Box sx={{ overflow: "hidden" }}>
                        ...
                        <Typography sx={{ color: SIDEBAR_STYLES.text, fontSize: 11, whiteSpace: "nowrap" }}>
                            Management System
                        </Typography>
                    </Box>
                )}
            ...
            >
                ...
                {open && (
                    <Box sx={{ overflow: "hidden", flexGrow: 1 }}>
                        <Typography sx={{ color: "white", fontSize: 13, fontWeight: 600, whiteSpace: "nowrap" }}>
                ...
                {open && (
                    <Tooltip title="Logout">
                        <IconButton 
                            ...
                            sx={{ color: SIDEBAR_STYLES.text, "&:hover": { color: "#ef4444" } }}
                        >
